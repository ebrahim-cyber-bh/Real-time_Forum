...
func CreatePostHandler(db *database.DBWrapper) http.HandlerFunc {
	return AuthRequired(func(w http.ResponseWriter, r *http.Request) {
		...
		if p.Category == "" || p.Title == "" || p.Content == "" {
			helpers.Error(w, "Category, title, and content are required", http.StatusBadRequest, fmt.Errorf("missing required fields"))
			return
		}

		if len([]rune(p.Content)) > 500 {
			helpers.Error(w, "Post content cannot exceed 500 characters", http.StatusBadRequest, fmt.Errorf("content too long"))
			return
		}
...
func GetPostsHandler(db *database.DBWrapper) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		posts, err := helpers.GetPosts(db.DB.DBConn, category)
		if err != nil {
...
func GetPostsByUserID(db *sql.DB, userID string) ([]models.Post, error) {
	rows, err := db.Query(`
		SELECT p.id, p.user_id, p.category, p.title, p.content, p.created_at, u.username, COUNT(c.id) AS comment_count
		FROM posts p
		JOIN users u ON p.user_id = u.id
		LEFT JOIN comments c ON c.post_id = p.id
		WHERE p.user_id = ?
		GROUP BY p.id, p.user_id, p.category, p.title, p.content, p.created_at, u.username
	...
	for rows.Next() {
		var p models.Post
		err := rows.Scan(&p.ID, &p.UserID, &p.Category, &p.Title, &p.Content, &p.CreatedAt, &p.UserName, &p.CommentCount)
		if err != nil {
			return nil, err
		}
...
func CreateCommentHandler(db *database.DBWrapper) http.HandlerFunc {
	return AuthRequired(func(w http.ResponseWriter, r *http.Request) {
		...
		if c.Content == "" {
			helpers.Error(w, "Comment content is required", http.StatusBadRequest, fmt.Errorf("empty comment"))
			return
		}
		if len([]rune(c.Content)) > 250 {
			helpers.Error(w, "Comment cannot exceed 250 characters", http.StatusBadRequest, fmt.Errorf("comment too long"))
			return
		}

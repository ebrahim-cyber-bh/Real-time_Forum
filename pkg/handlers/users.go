...
func GetUsersListHandler(db *database.DBWrapper) http.HandlerFunc {
	return AuthRequired(func(w http.ResponseWriter, r *http.Request) {
		...
		// Return a safe payload (never include password hashes)
		safe := make([]map[string]interface{}, 0, len(usrs))
		for _, u := range usrs {
...
func OnlineUsersHandler(h *ws.Hub, db *database.DBWrapper) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Println("OnlineUsersHandler error (unauthorized):", err)
		...
		if err != nil {
			log.Println("OnlineUsersHandler error (history users):", err)
		...
		if err != nil {
			log.Println("OnlineUsersHandler error (all users):", err)
			http.Error(w, "Failed to fetch users", http.StatusInternalServerError)
			return
		}
		...
		for _, user := range uniqueUsers {
			...
			if err != nil {
				log.Println("OnlineUsersHandler error (latest message):", err)

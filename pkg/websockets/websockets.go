	...
	for {
		...
		case msg := <-h.broadcast:
			...
			case "private_message":
				...
				receiverClient, receiverOnline := h.clients[msg.ReceiverID]
				if !receiverOnline {
					log.Println("receiver is offline, rejecting message:", msg.ReceiverID)
					// Notify the sender that the receiver is offline
					if senderClient, ok := h.clients[msg.SenderID]; ok {
						senderClient.Send <- WSMessage{
							Type:    "error",
							Content: "User is offline. Message not sent.",
						}
					}
					continue
				}

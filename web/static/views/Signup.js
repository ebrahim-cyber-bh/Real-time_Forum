...
export default class extends AbstractView {
  ...
  async getHtml() {
    return /* HTML */ `
      <div
        class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 to-gray-900 w-full"
      >
        <div
          class="w-11/12 sm:w-3/5 max-w-lg rounded-xl shadow-xl p-6 sm:p-12 flex flex-col items-center justify-center border border-indigo-400"
        >
  ...
  async onMounted() {
    handleFormSubmit("signup-form", (data) => {
      if (parseInt(data.age, 10) < 14) {
        window.toast.show("Age must be at least 14", "error", { position: { horizontal: "right", vertical: "top" } });
        return;
      }
      if (data.username.length < 3) {
        window.toast.show("Username must be at least 3 characters", "error", { position: { horizontal: "right", vertical: "top" } });
        return;
      }
      if (data.password.length < 4) {
        window.toast.show("Password must be at least 4 characters", "error", { position: { horizontal: "right", vertical: "top" } });
        return;
      }
      customFetch(

...
export default class extends AbstractView {
  ...
  async getHtml() {
    const categoriesHTML = CATEGORIES.map(
        (category) => /* HTML */ ` <div class="relative flex mr-3 sm:mr-6 mb-2">
    ...
    return /* HTML */ `
      <form
        class="bg-gray-900 w-full max-w-2xl mx-auto rounded-lg shadow p-5 md:p-8 my-4"
        id="post-create-form"
      >
          <h2
          ...
          <div class="w-full mt-5">
            ...
            <textarea
              ...
              rows="3"
              maxlength="500"
              class="block w-full h-32 resize-none rounded-md mt-2 border-0 bg-gray-800 p-2 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6"
              required
            ></textarea>
            <div class="flex justify-end mt-1">
              <span id="content-counter" class="text-xs text-gray-400">0 / 500</span>
            </div>
          </div>
          ...
          <button
            type="submit"
            class="mt-6 rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-400 w-full transition-all"
          >
            Save
          </button>
        </form>
  ...
  async onMounted() {
    ...
    if (contentEl && counterEl) {
      contentEl.addEventListener("input", () => {
        ...
        counterEl.className = len > 500 ? "text-xs text-red-400" : "text-xs text-gray-400";
      });
    }

    handleFormSubmit("post-create-form", async (data) => {
      if (!data.category) {
        window.toast.show("Please select a category", "error");
        return;
      }

      if (data.content.length > 500) {
        window.toast.show("Post content cannot exceed 500 characters", "error");
        return;
      }
